...
$(BASE_IMAGE): Dockerfile.base
	$(DOCKER_BUILD) -t docker.io/ibm/rt-nodejs-build-$@-$(MACHINE_ARCH) -f $< .
...
$(TESTS): %:
	docker pull docker.io/ibm/rt-nodejs-build-$@-img-$(MACHINE_ARCH)
	docker run -e "NPROC=$(NPROC)" docker.io/ibm/rt-nodejs-build-$@-img-$(MACHINE_ARCH) $(features)
	docker system prune -f

define PUSH
	docker push docker.io/ibm/rt-nodejs-build-$(1)-$(MACHINE_ARCH);
endef
